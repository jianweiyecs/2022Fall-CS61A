...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls == 0:
        ...
        return result + roll_dice(num_rolls,dice)
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def strategy(score,opponent_scorre):
        return n
    return strategy
    # END PROBLEM 6
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    scores = []
    for i in range(1,11):
        score = make_averaged(roll_dice,total_samples)
        scores.append(score(i,dice))
    max_item = max(scores)
    return scores.index(max_item) + 1
    # END PROBLEM 9
